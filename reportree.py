...
def filter_samples_interest(samples, matrix, out):
	...
	with open(matrix, "r") as mx:
		with open(out + "_SAMPLES_OF_INTEREST_partitions_summary.tsv", "w+") as out:
			i = 0
			for line in mx.readlines():
				...
				if i == 0:
					print("SAMPLE_OF_INTEREST\t" + lin[0], file = out)
				else:
					...
					if len(samples_observed) >= 1:
						print(",".join(samples_observed) + "\t" + lin[0], file = out)
				i += 1
...
if __name__ == "__main__":
	...
	group0.add_argument("-m", "--metadata", dest="metadata", required=True, type=str, help="[MANDATORY] Metadata file (tsv format)")
	...
	#group0.add_argument("-vcf", "--vcf", dest="vcf", default="", required=False, type=str, help="[OPTIONAL] Input list of VCF files with all the variants to include in the analysis (txt format)")
	...
	group1 = parser.add_argument_group("Clustering details", "Clustering details")
	...
	group1.add_argument("--loci-called", dest="loci_called", required=False, default = "", help="[OPTIONAL] Minimum percentage of loci/positions called (e.g. '--loci-called 0.95' will only keep \
						in the profile matrix samples with > 95%% of alleles/positions, i.e. <= 5%% missing data). Code for missing data: 0 for profile matrix and N for sequence alignment.")
	...
	#group1.add_argument("--cluster_subset", dest="cluster_subset", action="store_true", help="Redo the clustering analysis using only the samples that belong to the genetic clusters of the \
	#					samples of interest (only valid for analysis == grapetree or HC)")
	...
	group2 = parser.add_argument_group("Partitioning with GrapeTree", "Specifications to get and cut minimum spanning trees")
	...
	group3 = parser.add_argument_group("Partitioning with HC", "Specifications to genetic clusters with hierarchical clustering")
	group3.add_argument("--HC-threshold", dest="HCmethod_threshold", required=False, default="single", help="List of HC methods and thresholds to include in the analysis (comma-separated). To \
	...
	group4 = parser.add_argument_group("Partitioning with TreeCluster", "Specifications to cut the tree with TreeCluster")
	group4.add_argument("--method-threshold", dest="method_threshold", required=False, default="root_dist,avg_clade-1", 
						...
						root_dist!")
	group4.add_argument("--support", dest="support", required=False, default="-inf", help="[OPTIONAL: see TreeCluster github for details] Branch support threshold") 
	group4.add_argument("--root-dist-by-node", dest="root_dist_by_node", required=False, action="store_false", help="[OPTIONAL] Set only if you WANT to cut the tree with root_dist method at each tree \
						node distance to the root (similar to root_dist at all levels but just for informative distances)")
	
	
	## reportree
	
	group5 = parser.add_argument_group("ReporTree metadata report", "Specific parameters to report clustering/grouping information associated to metadata")
	group5.add_argument("--columns_summary_report", dest="columns_summary_report", required=False, default="n_sequence,lineage,n_country,country,n_region,first_seq_date,last_seq_date,timespan_days", type=str, help="Columns \
	...
	group5.add_argument("--partitions2report", dest="partitions2report", required=False, default="all", type=str, help="Columns of the partitions table to include in a joint report \
	...
	group5.add_argument("--metadata2report", dest="metadata2report", required=False, default="none", help="Columns of the metadata table for which a separated summary report must be provided \
						(comma-separated)")
	group5.add_argument("-f", "--filter", dest="filter_column", required=False, default="", help="[OPTIONAL] Filter for metadata columns to select the samples to analyze. This must be specified \
						...
						commas/semicolons.")
	group5.add_argument("--sample_of_interest", dest="sample_of_interest", required=False, default="all", help="Comma-separated list of samples of interest for which summary reports will be \
						created. If none provided, only the summary reports comprising all samples will be generated.")
	group5.add_argument("--frequency-matrix", dest="frequency_matrix", required=False, default="no", help="[OPTIONAL] Metadata column names for which a frequency matrix will be generated. This \
	...
	group5.add_argument("--count-matrix", dest="count_matrix", required=False, default="no", help="[OPTIONAL] Same as '--frequency-matrix' but outputs counts and not frequencies")
	group5.add_argument("--mx-transpose", dest="mx_transpose", required=False, action="store_true", help="[OPTIONAL] Set ONLY if you want that the variable1 specified in '--frequency-matrix' \
						or in '--count-matrix' corresponds to the matrix first column.")
					
						
	## comparing partitions
	
	group6 = parser.add_argument_group("Stability regions", "Congruence analysis of cluster composition at all possible partitions to determine regions of cluster stability (automatically run if you set --partitions2report 'stability_regions'). WARNING! This option is planned to handle sequential partitions obtained with the same clustering method, such as a partitions table derived from cg/wgMLST data (from 1 to max allele threshold). Use it at your own risk, if you provide your own partitions table.")
	group6.add_argument("-AdjW", "--AdjustedWallace", dest="AdjustedWallace", action= "store", default=0.99, help="Threshold of Adjusted Wallace score to consider an observation for method \
						stability analysis [0.99]")
	group6.add_argument("-n", "--n_obs", dest="n_obs", action="store", default=5, help="Minimum number of sequencial observations that pass the Adjusted Wallace score to be considered a \
						'stability region' (i.e. a threshold range in which cluster composition is similar) [5]")
	group6.add_argument("-o", "--order", dest="order", action= "store", default=0, required=False, help="[Set only if you provide your own partitions table] Partitions order in the partitions \
						table (0: min -> max; 1: max -> min) [0]")
	group6.add_argument("--keep-redundants", dest="keep_redundants", action= "store_true", help="Set ONLY if you want to keep all samples of each cluster of the most discriminatory partition\
	...
	## others provided	--------------------
	...
	elif args.allele_profile != "" or args.alignment != "" or args.distance_matrix != "":
		...
		if args.allele_profile != "": # ALLELE ---> DIRECT INPUT
			...
			if args.distance_matrix != "": # needs to be elif
				...
				if args.subset:
					os.system("python " + reportree_path + "/scripts/alignment_processing.py -align " + args.alignment + " -o " + args.output + " -m " + args.metadata + " -f \"" + args.filter_column + "\"")
				else:
					os.system("python " + reportree_path + "/scripts/alignment_processing.py -align " + args.alignment + " -o " + args.output)
			...
			if not distance_matrix_input:
				if args.loci_called != "":
					if args.subset == True:
						...
						-d " + str(args.dist) + " -m " + args.metadata + " -f \"" + args.filter_column + "\" --loci-called " + args.loci_called)
					else:
						os.system("python " + reportree_path + "/scripts/partitioning_HC.py -a " + profile + " -o " + args.output + " --HC-threshold " + args.HCmethod_threshold + " \
						-d " + str(args.dist) + " --loci-called " + str(args.loci_called))
				else:
					if args.subset == True:
						os.system("python " + reportree_path + "/scripts/partitioning_HC.py -a " + profile + " -o " + args.output + " --HC-threshold " + args.HCmethod_threshold + " \
						-d " + str(args.dist) + " -m " + args.metadata + " -f \"" + args.filter_column + "\"")
					else:
						os.system("python " + reportree_path + "/scripts/partitioning_HC.py -a " + profile + " -o " + args.output + " --HC-threshold " + args.HCmethod_threshold + " \
						-d " + str(args.dist))
			else:
		...
		if os.path.exists(args.output + "_metadata_w_loci_called.tsv"): # deleting metadata with loci called because this info is in main metadata
			os.system("rm " + args.output + "_metadata_w_loci_called.tsv")
