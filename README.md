...
3. A **new behavior of '--loci-called'** when performing cluster zoom-in/subtree. Until v2.4.1, the '--loci-called' was applied in cluster zoom-in/subtree. This was sometimes leading to the exclusion of some samples of the cluster of interest in the zoom-in/subtree because they did not have enough loci called. To avoid this situation, **from v2.5.0 on**, the '--loci-called' is not applied in zoom-in/subtree analyses, i.e. all samples of a cluster are used for zoom-in/subtree.
...
```bash
docker pull insapathogenomics/reportree:v2.5.0
```

Run ReporTree:
```bash
docker run insapathogenomics/reportree:v2.5.0 reportree.py -h
```
