...
## News!
#### 2026.09.25 - ReporTree v2.7.0
We release a new version of ReporTree that besides fixing some small issues, also brings important new features:
1. ReporTree is now able to report the list of clusters at a given threshold where a sample could also be assigned, if only hamming distances were considered. This report can be obtained with the "--get-compatible-clusters" argument and is particularly useful when using a method other than single-linkage, such as GrapeTree or traditional maximum-likelihood phylogenetic analysis.
2. ReporTree now has "--update-cluster-names" mode. With this argument, ReporTree will provide updated cluster names in *metadata_w_partitions.tsv, instead of adding new columns with the prefix subset. This is particularly useful to integrate ReporTree in automated surveillance workflows.
...
- compatible_clusters.tsv - report of the distance-based compatible clusters where a sample could also be assigned.
...
- Or install a specific version by specifying the release you want (e.g. for v2.7.0):
```bash
docker pull insapathogenomics/reportree:v2.7.0
```

Run ReporTree:
```bash
docker run insapathogenomics/reportree:v2.7.0 reportree.py -h
```
...
ReporTree:
  ...
  -p PARTITIONS, --partitions PARTITIONS
                        Partitions file (tsv format) - 'partition' represents the method-threshold at which clustering information was obtained
...
ReporTree cluster nomenclature:
  ...
  --update-cluster-names
                        [OPTIONAL] Activate update clusters mode. If you set this argument, and the metadata you provided has columns names that correspond to partition thresholds from a previous ReporTree run using the same analysis method as in the current run, ReporTree will
                        provide updated cluster names in the *metadata_w_partitions.tsv, instead of adding new columns with the prefix subset. This argument is only applicable if you provide a metadata file. To use this argument you do not need to provide a nomenclature file
                        nor request a nomenclature code. Please be careful when using this argument in combination with '--subset' bacause ReporTree will perform a run on the subset samples you selected AND update the *metadata_w_partitions.tsv with the new cluster names
...
ReporTree metadata report:
  ...
  --get-compatible-clusters COMPATIBLE_CLUSTERS
                        [OPTIONAL] Get the list of clusters at a given threshold where a sample could also be assigned, if hamming distances were considered. Partition thresholds can be indicated in this argument following the same rules as the arguments '-thr' and '-pct_thr'
  ...
  --zoom-all ZOOM_ALL   [OPTIONAL and only available for --analysis grapetree or HC] Repeat the analysis using only the samples that belong to each cluster identified at a given distance threshold. This argument takes as input only one threshold (partition) for which you want
                        the zoom-in. This threshold can be indicated in this argument following the same rules as the arguments '-thr' and '-pct_thr' for GrapeTree or '--HC-threshold' and '--pct-HC-threshold' for HC. This argument requires that a metadata table was provided
                        with '-m'. The argument '--loci-called' is not applied in the zoom-in analysis, i.e. all samples of the cluster are included. Default: no zoom-in.
  --unzip               [OPTIONAL and only available for --analysis grapetree or HC] Provide the outputs of '--zoom-cluster-of-interest', '--subtree-of-interest' and '--zoom-all' in unzipped format.
...
At an initial stage, this work was supported by funding from the European Union’s Horizon 2020 Research and Innovation programme under grant agreement No 773830: One Health European Joint Programme. 
ReporTree development and maintnance is currently supported by:
- National funds through FCT - Foundation for Science and Technology, I.P., in the framework of Individual CEEC 2022.00851.CEECIND/CP1748/CT0001 (doi: 10.54499/2022.00851.CEECIND/CP1748/CT0001)
